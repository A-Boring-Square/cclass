...
METHOD_IMPL_NO_ARGS(UInt32, void, Not) {
    self->_data=~self->_data;
}
METHOD_IMPL(UInt32, void, Shl, uint32_t bits) {
    if(bits>31) PANIC_RUNTIME("Shift left out of bounds");
    self->_data<<=bits;
}
METHOD_IMPL(UInt32, void, Shr, uint32_t bits) {
    if(bits>31) PANIC_RUNTIME("Shift right out of bounds");
    self->_data>>=bits;
}
CONSTRUCTOR_OF(UInt32){
    UInt32* u32=(UInt32*)malloc(sizeof(UInt32));
    if(!u32) PANIC_RUNTIME("Failed to allocate UInt32");
    return u32;
}
