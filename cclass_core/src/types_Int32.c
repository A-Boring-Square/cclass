...
METHOD_IMPL_NO_ARGS(Int32, void, Not) {
    self->_data = ~self->_data;
}
METHOD_IMPL(Int32, void, Shl, int32_t bits) {
    if(bits<0||bits>31)PANIC_RUNTIME("Shift left out of bounds (0-31)");
    self->_data = (int32_t)((uint32_t)self->_data<<bits);
}
METHOD_IMPL(Int32, void, Shr, int32_t bits) {
    if(bits<0||bits>31)PANIC_RUNTIME("Shift right out of bounds (0-31)");
    self->_data = (int32_t)((uint32_t)self->_data>>bits);
}
CONSTRUCTOR_OF(Int32) {
    Int32* i32 = (Int32*)malloc(sizeof(Int32));
    if(!i32)PANIC_RUNTIME("Failed to allocate Int32");
    return i32;
}
