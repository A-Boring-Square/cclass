...
METHOD_IMPL_NO_ARGS(UInt8, void, Not) { self->_data = ~self->_data; }
METHOD_IMPL(UInt8, void, Shl, uint8_t bits) { if(bits > 7) PANIC_RUNTIME("Shift left out of bounds"); self->_data <<= bits; }
METHOD_IMPL(UInt8, void, Shr, uint8_t bits) { if(bits > 7) PANIC_RUNTIME("Shift right out of bounds"); self->_data >>= bits; }

CONSTRUCTOR_OF(UInt8) { UInt8* u8 = (UInt8*)malloc(sizeof(UInt8)); if(!u8) PANIC_RUNTIME("Failed to allocate UInt8"); return u8; }
