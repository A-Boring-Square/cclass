...
while(n) { UInt16 tmp={._data=acc}; acc=op(&tmp,n); n=va_arg(args,UInt16*); }
return acc;
}
...
METHOD_IMPL_NO_ARGS(UInt16, void, Not) { self->_data=~self->_data; }
METHOD_IMPL(UInt16, void, Shl, uint16_t bits) { if(bits>15) PANIC_RUNTIME("Shift left out of bounds"); self->_data<<=bits; }
METHOD_IMPL(UInt16, void, Shr, uint16_t bits) { if(bits>15) PANIC_RUNTIME("Shift right out of bounds"); self->_data>>=bits; }

CONSTRUCTOR_OF(UInt16){ UInt16* u16=(UInt16*)malloc(sizeof(UInt16)); if(!u16) PANIC_RUNTIME("Failed to allocate UInt16"); return u16; }
