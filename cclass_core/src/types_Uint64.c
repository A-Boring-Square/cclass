...
static uint64_t div_op(UInt64* a, UInt64* b) {if (b->_data == 0) PANIC_RUNTIME("Division by zero");return a->_data / b->_data;}
static uint64_t mod_op(UInt64* a, UInt64* b) {if (b->_data == 0) PANIC_RUNTIME("Modulo by zero");return a->_data % b->_data;}
...
DESTRUCTOR_OF(UInt64) { free(self); }
